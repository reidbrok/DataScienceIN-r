...
conflict = conflict_data_raw %>% filter (year <= 2019 & year >= 2000) %>% group_by(year,ISO) %>% summarize(conflict_country_year = sum(best)) %>% mutate(`armed conflict` = ifelse(conflict_country_year >= 25, 1, 0))
